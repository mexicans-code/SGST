...
const localChatKey = (estId, anfId) => `sgst_mock_chat_${estId}_${anfId}`;

const loadLocalChat = (estId, anfId) => {
    try {
        return JSON.parse(localStorage.getItem(localChatKey(estId, anfId)) || '[]');
    } catch (_err) {
        void _err;
        return [];
    }
...
const saveLocalChat = (estId, anfId, messages) => {
    try {
        localStorage.setItem(localChatKey(estId, anfId), JSON.stringify(messages));
    } catch (_err) {
        void _err;
    }
};
...
export default function ChatModal({ isOpen, onClose, anfitrion, establecimiento }) {
    ...
    useEffect(() => {
        if (isOpen && anfitrion && establecimiento) {
            ...
            socketRef.current = io(`https://chat-production-f6677.up.railway.app`, {
                auth: {
                    token: token
                },
            ...
            socketRef.current.on("connect_error", (error) => {
                ...
                setConnected(false);
                // Modo demostración: si el servidor de chat no existe, operar localmente
                fallbackTimer = setTimeout(() => {
                    if (!socketRef.current?.connected) {
                        console.log("💬 Modo demo: chat local activado");
                        setConnected(true);
                        setMockMode(true);
                        setMessages(loadLocalChat(establecimientoId, anfitrionId));
                    }
                }, 1500);
            });
    ...
    const handleSendMessage = (e) => {
        ...
        // Modo demostración: guardar el mensaje localmente
        if (mockMode) {
            const currentUser = getCurrentUser();
            const message = {
                id: Date.now(),
                user_id: currentUser?.id,
                user_name: currentUser?.nombre || 'Tú',
                content: newMessage.trim(),
                created_at: new Date().toISOString()
            };
            setMessages(prev => {
                const next = [...prev, message];
                saveLocalChat(establecimientoId, anfitrionId, next);
                return next;
            });
            setNewMessage("");
            return;
